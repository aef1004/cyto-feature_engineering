...
First, the packages must be loaded. All packages can be installed from CRAN or Bioconductor.
```{r warning= FALSE}
library(openCyto)
library(flowClust)
library(data.table)
...
[Amy, are we still using all the above packages? If not, it might be a good time to 
clean up ones we don't need from this and the other Rmd documents in this project.]
...
```{r}
# [Amy, could you add a note in comments to the beginning of each new function to 
# specify what that function does? For example, here it looks like the function 
# renames columns, omits rows with all values missing, and filters out rows where
# the side scatter is equal to its maximum value (because...). It inputs a tidy
# dataframe with flow cytometry data from one sample and outputs a cleaned version 
# of that dataframe.]
...
Initial gating of FMOs [Amy, if we're posting this as a supplement to the manuscript, we'll want to add some more details here to get users started. For example, we should explain again here that the data file named [x] would need to be obtained (by email from us? from another repository with larger size limits?) and put in a subdirectory called "Data/flow/BCG_Survival", for example.]

```{r}
# read in the gating strategy
ws <- list.files("../Data/flow/BCG_Survival", pattern = "AmyGating_intracellular.csv", full = TRUE)

# only for viewing that read in [Amy, could you clarify this comment?]
dtTemplate <- fread(ws) 

initial_gate <- gatingTemplate(ws) 

# read in the FMOs [Amy, I think I'll need to get these files from you. I don't think I have them
# in the earlier data directory you shared.]
