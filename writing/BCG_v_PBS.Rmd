...
[Amy, the above code I think could be used to replace the code you have below as a 
way to create a tidy dataframe from the 'flowSet' data. Once you get the chance
to look through this, if you agree we can delete the code below (right now I've just
set it to not evaluate).]
...
[Again, we can delete the following if we're happy with the function replacement above. 
I have it set to not evaluate right now.]
...
[Amy, could you add some text saying what we're looking at with this next plot?]
...
Need to
- make dataframe that only contains CD3 marker MFI for the CD3 FMO, only CD103 for CD103 FMO etc.
- Create data frame with the different thresholds

```{r}
# try to do the thresholds
FMO_gated_df %>%
  clean_FMO_after_gate() %>%
  group_by(filename) %>%
  summarise(CD3_threshold = quantile(CD3, 0.99),
            CD103_threshold = quantile(CD103, 0.99))
## [Amy, should the above code be assigning somewhere? Or are you just
## printing out these results to check something?]

# try to make the data readable with ggplot
FMO_gated_for_facet <- FMO_gated_df %>%
  clean_FMO_after_gate() %>%
  gather(key = "Marker", value = "MFI", -filename, -SSC.A) 

# not plotting correctly
ggplot(FMO_gated_for_facet, aes(x = MFI, y = `SSC.A`)) +
...
```{r fig.width = 11, fig.height=4, warning=FALSE}
...
grid.arrange(p1, p2, p3, p5, p6, p7, p8, p9,
             ...
             nrow = 3, left = textGrob("SSC-A", rot = 90, vjust = 1, gp=gpar(fontsize=24, lineheight=1)))
```
...
D30_pops_CFUs <- pops_CFUs %>%
  dplyr::filter(`Timepoint` == 30)

ggplot(D30_pops_CFUs, aes(percentage, CFU, color = Group)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "#3F4788FF") +
  facet_wrap(~population, scales = "free_x", ncol = 9) +
  stat_smooth_func(geom= "text", method = "lm", hjust = 0, parse = TRUE) +
...
Want to try to add different colors to facets
```{r}
...
ggplot(pops_CFUs, aes(percentage, CFU)) +
  geom_point() +
  facet_wrap(~population, scales = "free_x", ncol = 9) +
  geom_smooth(method = "lm", formula = formula, se = FALSE, color = "#3F4788FF") +
  stat_poly_eq(formula = formula, parse = TRUE, size = 2) +
    stat_fit_glance(method = 'lm', method.args = list(formula= formula), geom = 'text',
                    aes(label = paste("p-value = ", signif(..p.value.., digits = 4), sep = "")),
                        label.x.nps = "right", label.y.npc = 0.35, size = 3) +
  ylab("log10 CFU") +
  ggtitle("Population and CFU Linear regression") +
  theme(axis.text.x = element_text(size = 9, hjust = 1),
        axis.text.y = element_text(size = 9),
...
ggplot(pops_CFUs, aes(percentage, CFU)) +
  ...
  stat_fit_glance(method = "lm", method.args = list(formula = formula),
                  ...
                  aes(label = paste("p-value", signif(..p.value.., digits = 2))), size = 2)
