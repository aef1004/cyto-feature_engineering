...
Split the data into CD3+CD4+CD8- CD3+CD4-CD8+, double negative (CD4-CD8-), and double positive (CD4+CD8+) for prettier plots

(Maybe splitting isn't the best way to do it - I could also filter or do what I did for the Colombia dataset)
...
tester123_annot <- tester123 %>%
  select(resident, cell, lineage) %>%
  mutate(population = c(paste0("Pop", 1: nrow(sample_populations)))) %>%
  column_to_rownames("population")


pheatmap(tester123[, 4:21], cluster_rows = FALSE, cluster_cols = FALSE,
           labels_row = paste("Pop", 1:nrow(sample_populations)), 
           annotation_row = tester123_annot,  gaps_row = c(17, 29, 35),
