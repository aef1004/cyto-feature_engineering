...
# read in. 
FMO_fcsFiles <- list.files("../Data/flow/BCG_v_PBS/Tcell/FMO_final", 
...
ncfs_FMO <- read.ncdfFlowSet(FMO_fcsFiles, channel_alias = data.frame(alias = c("Zombie NIR-A"), channels = c("Zombie Nir-A", "Zombie NIR-A"))) 
...
##########################
# tried to convert to tidy data, but ggplot still plots all of the FMO files for each marker
testing <- FMO_gated_data %>%
  clean_FMO_after_gate() %>%
  gather(key = "marker", value = "MFI", -filename, -`SSC-A`)

ggplot(testing, aes(x = MFI, y = `SSC-A`)) +
...
# try to use purrr to plot the 
library(cowplot)
clean_test <- FMO_gated_data %>%
  clean_FMO_after_gate() 


gg_fun <- function(x, y) {
  ggplot(clean_data, aes(x = .data[[x]], y = .data[[y]])) +
  geom_hex(bins = 100, na.rm = TRUE) +
   scale_fill_viridis_c() +
   ylab("") +
  theme_gray()+
...
markers <- names(clean_test[2:21])


FMO_plots <- map(markers, ~gg_fun(.x, "SSC-A")+
                   facet_wrap(~filename))

cowplot::plot_grid(plotlist = FMO_plots)

############
# this works for plotting the right data!!! 
# now I just need to figure out how to do the vertical line....
# Problem with CD44 because it's pulling in the CD4 data as well...
testing <- FMO_gated_data %>%
  clean_FMO_after_gate() %>%
  gather(key = "marker", value = "MFI", -filename, -`SSC-A`)

right <- testing %>%
  filter(str_detect(testing$filename, testing$marker)) %>%
  filter(marker != "CD69") %>%
  filter(marker != "FoxP3")
...
right_quantile <- full_join(right, add_quantile)
...
p4 <- FMO_ggplot(FoxP3_FMO, FoxP3_FMO$FoxP3, "FoxP3") # this plot is removed because we are not going to be using this marker
p5 <- FMO_ggplot(CD27_FMO, CD27_FMO$CD27, "CD27")
p6 <- FMO_ggplot(CD28_FMO, CD28_FMO$CD28, "CD28")
p7 <- FMO_ggplot(CD44_FMO, CD44_FMO$CD44, "CD44")
p8 <- FMO_ggplot(IL_10_FMO, IL_10_FMO$IL_10, "IL_10")
p9 <- FMO_ggplot(CD62L_FMO, CD62L_FMO$CD62L, "CD62L")
p10 <- FMO_ggplot(CD69_FMO, CD69_FMO$CD69, "CD69") # this plot is removed because we are not going to be using this marker
p11 <- FMO_ggplot(CD103_FMO, CD103_FMO$CD103, "CD103")
p12 <- FMO_ggplot(CD122_FMO, CD122_FMO$CD122, "CD122")
p13 <- FMO_ggplot(CD153_FMO, CD153_FMO$CD153, "CD153")
p14 <- FMO_ggplot(CTLA4_FMO, CTLA4_FMO$CTLA4, "CTLA4")
p15 <- FMO_ggplot(IL_17_FMO, IL_17_FMO$IL_17, "IL_17")
p16 <- FMO_ggplot(IFN_FMO, IFN_FMO$IFN, "IFN")
p17 <- FMO_ggplot(TNF_FMO, TNF_FMO$TNF, "TNF")
p18 <- FMO_ggplot(KLRG1_FMO, KLRG1_FMO$KLRG1, "KLRG1")
p19 <- FMO_ggplot(PD1_FMO, PD1_FMO$PD1, "PD1")
p20 <- FMO_ggplot(Sca1_FMO, Sca1_FMO$Sca1, "Sca1")


grid.arrange(p1, p2, p3, p5, p6, p7, p8, p9,
             p11, p12, p13, p14, p15, p16, p17, p18, p19, p20, 
             nrow = 5, left = textGrob("SSC-A", rot = 90, vjust = 1, gp=gpar(fontsize=20, lineheight=1)))
```
...
fcsFiles <- list.files("../Data/flow/BCG_v_PBS/Tcell/Tcell_final/", 
                       pattern = ".fcs", full = TRUE)
...
ncfs <- read.ncdfFlowSet(fcsFiles, channel_alias = data.frame(alias = c("Zombie NIR-A"), channels = c("Zombie Nir-A", "Zombie NIR-A"))) 
