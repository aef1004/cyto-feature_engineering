...
# remove mean_se
# too many points - like a lot of extra
ggline(x = "Group", y = "CFU", color = "Group", alpha = 0.5,
         add = c("jitter"), data = lung_CFU, 
       font.label = list(size = 30, face = "plain")) +
  facet_wrap("Day", ncol = length(unique(lung_CFU$Day)), 
             labeller = label_both)+
  stat_pvalue_manual(data = lung_CFU_signif, 
...
# add facet to ggline rather than separate functoin
# still does not work
ggline(x = "Group", y = "CFU", color = "Group", alpha = 0.5,
         data = lung_CFU, add = "jitter", facet.by = "Day",
       font.label = list(size = 30, face = "plain")) +
  stat_pvalue_manual(data = lung_CFU_signif, 
                     label = "star",
                     xmin = "group1", xmax = "group2",
