...
# this code is from OpenCyto Github 
# this function is from RGLab https://github.com/RGLab/openCyto/blob/0ec1218427627b9ee092f8d39e6ab46d46e7e2f8/R/bayes-flowClust.R
...
sc = single colors
These single colors are using beads, so we will not gate for live and leukocytes (just take the isnglets)
```{r}

ws <- list.files("../Data/flow/Colombia", pattern = "Colombia_gating.csv", full = TRUE)
...
sc_fcsFiles <- list.files("../Data/flow/Colombia/Single_Colors", 
...
gate_mindensity(sc_gated_data[[1]], "PE-A")

```

Test logicle transform
...
for (i in 1:length(sc_gated_data)) {
  lgcl <- estimateLogicle(sc_gated_data[[i]], channels = c( "FITC-A", "PE-A",
                                                 "PE-Cy5-A", "PE-Cy7-A", "APC-A"))
  
  sc_data_test <- transform(sc_gated_data[[i]], lgcl) %>%
    exprs() %>%
    data.frame() %>%
...
I can no longer use 'quantile` for the data because the single color beads have both positive and negative values. I will try to use OpenCyto to find the minimum density between two peaks. The problem I may encounter is that I need to pull out the x-axis value for the OpenCyto threshold.
...
```{r}
fcsFile <- list.files("../Data/flow/Colombia/", 
                       pattern = ".fcs", full = TRUE)
...
for (i in 1:length(gated_data)) {
   lgcl_data <- estimateLogicle(gated_data[[i]], channels = c( "FITC-A", "PE-A",
                                                 "PE-Cy5-A", "PE-Cy7-A", "APC-A"))
...
pheatmap(total_phenotypes, cluster_rows = FALSE, cluster_cols = FALSE, 
         color = viridis_colors, 
           main ="All Colombia Populations")

nrow(total_phenotypes) 

# view the specific cell phenotypes and percentages
...
Correlation Plot
- cannot do because only have 1 sample
```{r}
...
#superheat(corr, title = "Population Correlation", row.title = "Populations", column.title = "Populations", grid.hline = FALSE, grid.vline = FALSE)
```
...
Test all_gated to get accurate pops
```{r}
manual_gating <- read_xlsx("../Data/flow/Colombia/manual_gating.xlsx") %>%
...
ggplot(comparison, aes(x = percent_pipeline, y = percent_manual)) +
  ...
  ggtitle("Colombia Data: Manual vs. Pipeline Percentage") +
