...
all_gated_cell_ID <- all_gated_clean %>%
  mutate(cell_ID = c(1:nrow(all_gated_clean)))
  
all_gated <- all_gated_cell_ID %>%
...
```{r}
# filter to the 36 populations in the smaller plot
...
sample_populations <- all_gated_tSNE %>%
...
# need to add back in the cell_ID
left_join(all_gated_tSNE_36_pops_ID, all_gated, by = c("CD3", "CD4", "CD8", "CD44", "CD103", "Sca1", "IL_17", "CTLA4", "CD27", 
           "CD153", "KLRG1", "IFN",  "CD122", "PD1", "CD62L", "IL_10", "CD28", "TNF"))

# add back in the original data
left_join(all_gated_tSNE_36_pops_ID, all_gated_cell_ID, by = "cell_ID")
...
# downsampling to the 36 populations
all_gated_tSNE_36_pops <- all_gated %>%
  mutate(lineage = ifelse(CD4 == 0 & CD8 ==0, "Double Negative",
                          ifelse(CD4 == 1 & CD8 ==0,"T Helper",
                          ifelse(CD4 ==0 & CD8 ==1, "Cytotoxic T", 
                                 "Double Positive"))),
...
colors = rainbow(length(unique(all_gated_tSNE_36_pops$lineage_cell)))
names(colors) = unique(all_gated_tSNE_36_pops$lineage_cell)

tsne_FE_36_pops <- Rtsne(all_gated_tSNE_36_pops[,-c(1:2)], dims = 2, perplexity=30, verbose=TRUE, max_iter = 500, check_duplicates = FALSE)

data_plot_FE_36_pops <- as.data.frame(tsne_FE_36_pops$Y)
colnames(data_plot_FE_36_pops) <- c("tSNE_1", "tSNE_2")

ggplot(data_plot_FE_36_pops, aes(x = tSNE_1, y = tSNE_2, col = all_gated_tSNE_36_pops$lineage_cell)) + 
  geom_point(size = 0.2) + 
  coord_fixed(ratio = 1) +
  xlab("tSNE 1") +
  ylab("tSNE 2") +
...
# downsampling to the 36 populations
# Look at just the 4 populations
all_gated_tSNE_36_pops_lineage <- all_gated %>%
  mutate(lineage = ifelse(CD4 == 0 & CD8 ==0, "Double Negative",
                          ifelse(CD4 == 1 & CD8 ==0,"T Helper",
                          ifelse(CD4 ==0 & CD8 ==1, "Cytotoxic T", 
                                 "Double Positive"))),
         cell = ifelse(CD44 == 0 & CD62L == 1 & Sca1 == 1, "TSCM", 
                       ifelse(CD44 ==1 & CD62L==0, "Effector", 
                              ifelse(CD44 == 0 & CD62L == 1 & Sca1 == 0, "Naive",
                                     ifelse(CD44==1 & CD62L ==1, "Central Mem",
                                     "Unknown"))))) %>%
  select(filename, lineage, CD3, CD4, CD8, CD44, CD103, Sca1, IL_17, CTLA4, CD27, 
           CD153, KLRG1, IFN,  CD122, PD1, CD62L, IL_10, CD28,
           TNF) 

# change the 1's to range from 1 to 1.1 and o's from 0 to 0.1 to see if we can get a better tSNE plot
library(data.table)
setDT(all_gated_tSNE_36_pops_lineage)[CD3 == 0, CD3 := runif(.N, min=0.0001, max=0.01)]
setDT(all_gated_tSNE_36_pops_lineage)[CD3 == 1, CD3 := runif(.N, min=1.0001, max=1.01)]
setDT(all_gated_tSNE_36_pops_lineage)[CD4 == 0, CD4 := runif(.N, min=0.0001, max=0.01)]
setDT(all_gated_tSNE_36_pops_lineage)[CD4 == 1, CD4 := runif(.N, min=1.0001, max=1.01)]
setDT(all_gated_tSNE_36_pops_lineage)[CD8 == 0, CD8 := runif(.N, min=0.0001, max=0.01)]
setDT(all_gated_tSNE_36_pops_lineage)[CD8 == 1, CD8 := runif(.N, min=1.0001, max=1.01)]
setDT(all_gated_tSNE_36_pops_lineage)[CD44 == 0, CD44 := runif(.N, min=0.0001, max=0.01)]
setDT(all_gated_tSNE_36_pops_lineage)[CD44 == 1, CD44 := runif(.N, min=1.0001, max=1.01)]
setDT(all_gated_tSNE_36_pops_lineage)[CD103 == 0, CD103 := runif(.N, min=0.0001, max=0.01)]
setDT(all_gated_tSNE_36_pops_lineage)[CD103 == 1, CD103 := runif(.N, min=1.0001, max=1.01)]
setDT(all_gated_tSNE_36_pops_lineage)[Sca1 == 0, Sca1 := runif(.N, min=0.0001, max=0.01)]
setDT(all_gated_tSNE_36_pops_lineage)[Sca1 == 1, Sca1 := runif(.N, min=1.0001, max=1.01)]
setDT(all_gated_tSNE_36_pops_lineage)[IL_17 == 0, IL_17 := runif(.N, min=0.0001, max=0.01)]
setDT(all_gated_tSNE_36_pops_lineage)[IL_17 == 1, IL_17 := runif(.N, min=1.0001, max=1.01)]
setDT(all_gated_tSNE_36_pops_lineage)[CTLA4 == 0, CTLA4 := runif(.N, min=0.0001, max=0.01)]
setDT(all_gated_tSNE_36_pops_lineage)[CTLA4 == 1, CTLA4 := runif(.N, min=1.0001, max=1.01)]
setDT(all_gated_tSNE_36_pops_lineage)[CD27 == 0, CD27 := runif(.N, min=0.0001, max=0.01)]
setDT(all_gated_tSNE_36_pops_lineage)[CD27 == 1, CD27 := runif(.N, min=1.0001, max=1.01)]
setDT(all_gated_tSNE_36_pops_lineage)[CD153 == 0, CD153 := runif(.N, min=0.0001, max=0.01)]
setDT(all_gated_tSNE_36_pops_lineage)[CD153 == 1, CD153 := runif(.N, min=1.0001, max=1.01)]
setDT(all_gated_tSNE_36_pops_lineage)[KLRG1 == 0, KLRG1 := runif(.N, min=0.0001, max=0.01)]
setDT(all_gated_tSNE_36_pops_lineage)[KLRG1 == 1, KLRG1 := runif(.N, min=1.0001, max=1.01)]
setDT(all_gated_tSNE_36_pops_lineage)[IFN == 0, IFN := runif(.N, min=0.0001, max=0.01)]
setDT(all_gated_tSNE_36_pops_lineage)[IFN == 1, IFN := runif(.N, min=1.0001, max=1.01)]
setDT(all_gated_tSNE_36_pops_lineage)[CD122 == 0, CD122 := runif(.N, min=0.0001, max=0.01)]
setDT(all_gated_tSNE_36_pops_lineage)[CD122 == 1, CD122 := runif(.N, min=1.0001, max=1.01)]
setDT(all_gated_tSNE_36_pops_lineage)[PD1 == 0, PD1 := runif(.N, min=0.0001, max=0.01)]
setDT(all_gated_tSNE_36_pops_lineage)[PD1 == 1, PD1 := runif(.N, min=1.0001, max=1.01)]
setDT(all_gated_tSNE_36_pops_lineage)[CD62L == 0, CD62L := runif(.N, min=0.0001, max=0.01)]
setDT(all_gated_tSNE_36_pops_lineage)[CD62L == 1, CD62L := runif(.N, min=1.0001, max=1.01)]
setDT(all_gated_tSNE_36_pops_lineage)[IL_10 == 0, IL_10 := runif(.N, min=0.0001, max=0.01)]
setDT(all_gated_tSNE_36_pops_lineage)[IL_10 == 1, IL_10 := runif(.N, min=1.0001, max=1.01)]
setDT(all_gated_tSNE_36_pops_lineage)[CD28 == 0, CD28 := runif(.N, min=0.0001, max=0.01)]
setDT(all_gated_tSNE_36_pops_lineage)[CD28 == 1, CD28 := runif(.N, min=1.0001, max=1.01)]
setDT(all_gated_tSNE_36_pops_lineage)[TNF == 0, TNF := runif(.N, min=0.0001, max=0.01)]
setDT(all_gated_tSNE_36_pops_lineage)[TNF == 1, TNF := runif(.N, min=1.0001, max=1.01)]

# tried doing this, but it won't work (also tried mutate_if)
all_gated_tSNE_36_pops_lineage <- all_gated_tSNE_36_pops_lineage %>%
  mutate_at("CD4", funs(replace(., 0, runif(.N, min = 0.01, max = 0.1))))
  mutate_at("CD3", funs(replace(., 1, runif(nrow(all_gated), min = 1, max = 1.1)))) 
  
colnames(all_gated_tSNE_36_pops_lineage)
colors = rainbow(length(unique(all_gated_tSNE_36_pops_lineage$lineage)))
names(colors) = unique(all_gated_tSNE_36_pops_lineage$lineage)

tsne_FE_36_pops_lineage <- Rtsne(all_gated_tSNE_36_pops_lineage[,-c(1:2)], dims = 2, perplexity=30, verbose=TRUE, max_iter = 500, check_duplicates = FALSE)

data_plot_FE_36_pops_lineage <- as.data.frame(tsne_FE_36_pops_lineage$Y)
colnames(data_plot_FE_36_pops_lineage) <- c("tSNE_1", "tSNE_2")

ggplot(data_plot_FE_36_pops_lineage, aes(x = tSNE_1, y = tSNE_2, col = all_gated_tSNE_36_pops_lineage$lineage)) + 
  geom_point(size = 0.2) + 
  coord_fixed(ratio = 1) +
  xlab("tSNE 1") +
  ylab("tSNE 2") +
...
Using the feature engineered data from before to analyze this (with the thresholds to name the data)

# try to see how long it takes to run when using only 46,937 rows (the same number of rows when feature engineering the 36 pops)
...
normal_tSNE <- all_gated_df %>%
  ...
  dplyr::filter(`SSC.A` != max(`SSC.A`)) %>%
  select(-Zombie.NIR.A, -AF.A, -SSC.A, -FoxP3, -CD69) %>%
    mutate(lineage = ifelse(CD4 < quantile(CD4_FMO$CD4, 0.99) & CD8 < quantile(CD8_FMO$CD8, 0.99), "Double Negative",
                          ifelse(CD4 > quantile(CD4_FMO$CD4, 0.99) & CD8 < quantile(CD8_FMO$CD8, 0.99),"T Helper",
                          ifelse(CD4 <quantile(CD4_FMO$CD4, 0.99) & CD8 >quantile(CD8_FMO$CD8, 0.99), "Cytotoxic T", 
                                 "Double Positive"))),
         cell = ifelse(CD44 < quantile(CD44_FMO$CD44, 0.99) & CD62L > quantile(CD62L_FMO$CD62L, 0.99) & Sca1 >quantile(Sca1_FMO$Sca1, 0.99), "TSCM", 
                       ifelse(CD44 > quantile(CD44_FMO$CD44, 0.99) & CD62L < quantile(CD62L_FMO$CD62L, 0.99), "Effector", 
                              ifelse(CD44 <quantile(CD44_FMO$CD44, 0.99) & CD62L > quantile(CD62L_FMO$CD62L, 0.99) & Sca1 <quantile(Sca1_FMO$Sca1, 0.99), "Naive",
                                     ifelse(CD44 >quantile(CD44_FMO$CD44, 0.99) & CD62L >quantile(CD62L_FMO$CD62L, 0.99), "Central Mem",
                                     "Unknown"))))) %>%
  select(filename, lineage, IL_10:CD44) %>%
  unique() %>%
  filter(CD3 >quantile(CD3_FMO$CD3, 0.99)) %>%
  top_n(46937)
...
## Plotting
colors = rainbow(length(unique(normal_tSNE$lineage)))
names(colors) = unique(normal_tSNE$lineage)

data_plot_normal <- as.data.frame(tsne_normal$Y)
colnames(data_plot_normal) <- c("tSNE_1", "tSNE_2")

ggplot(data_plot_normal, aes(x = tSNE_1, y = tSNE_2, col = normal_tSNE$lineage)) + 
  geom_point(size = 0.2) + 
  coord_fixed(ratio = 1) + 
